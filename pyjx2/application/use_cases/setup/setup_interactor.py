...
class SetupInteractor:
    ...
    def execute(
        self, config: SetupConfig, logger: Callable[[str], None] | None = None
    ) -> SetupResult:
        def log(msg: str):
            if logger:
                logger(msg)
        ...
        log(
            f"Test Plan validado correctamente ({len(valid_plan_test_keys)} tests maestros encontrados)."
        )
        ...
        for exec_conf in config.test_executions:
            ...
            try:
                ...
                if exec_conf.mode == "create":
                    metrics.executions_created += 1
                else:
                    metrics.executions_reused += 1
            ...
            for set_conf in exec_conf.test_sets:
                ...
                try:
                    set_ticket = self.set_resolver.resolve(
                        set_conf, config.project_key, config.settings.clean_test_set_before, log
                    )
                    final_sets.append(set_ticket)
                    if set_conf.mode == "create":
                        metrics.sets_created += 1
                    else:
                        metrics.sets_reused += 1
                ...
                if set_conf.apply_source and set_conf.source:
                    raw_keys = self.source_resolver.resolve(
                        set_conf.source, valid_plan_test_keys, log
                    )

                    if not raw_keys:
                        if config.settings.on_empty_source == "fail":
                            raise ValueError(
                                "[FAIL FAST] Operación abortada: Entorno de source retornó 0 casos."
                            )
                        else:
                    ...
                    for k in raw_keys:
                        if k in unique_keys:
                            if config.settings.on_duplicate == "fail":
                                raise ValueError(
                                    f"[FAIL FAST] Elemento duplicado detectado prohibitivamente: {k}"
                                )
                        ...
                        if set_conf.test_case_mode == "clone":
                            ...
                            try:
                                ...
                                if config.settings.on_missing_test_case == "fail":
                                    raise ValueError(
                                        f"[FAIL FAST] Falla materializando clon de {t_key}: {e}"
                                    ) from e
                            ...
                            try:
                                ...
                                log(
                                    f"[WARN] Imposible inyectar etiqueta CICD-Pipeline a {t_key}: {e}"
                                )
        ...
        return SetupResult(
            test_executions=final_execs, test_sets=final_sets, tests=final_tests, metrics=metrics
        )
