...
def sync(
    ...
    if status.upper() not in valid_statuses:
        console.print(f"[bold red]Estado invalido:[/bold red] '{status}'." f" Valores validos: {', '.join(sorted(valid_statuses))}")
        raise typer.Exit(code=1)
    ...
    try:
        result = pjx.sync(execution_key=execution_key, folder=folder, status=status.upper(), status_overrides=overrides, upload_mode=upload_mode, allowed_extensions=allowed_ext, recursive=recursive, progress_callback=on_progress)
...
def docs() -> None:
    ...
    proc = None
    try:
        proc = subprocess.Popen(["mkdocs", "serve"], cwd=base_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
@app.command()
def tui(config: Optional[str] = _config_option()) -> None:
...
def encrypt_pass(password: str = typer.Argument(..., help="Contrasena en texto plano a cifrar.")) -> None:
...
def decrypt_pass(token: str = typer.Argument(..., help="Token cifrado que comienza con ENC:.")) -> None:
    from ..infrastructure.security.encryption import SymmetricEncryptionService
