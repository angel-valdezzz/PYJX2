...
app = typer.Typer(
    name="pyjx2",
    help="Herramienta de automatizacion Jira/Xray â€” prepara ejecuciones y sincroniza evidencias.",
...
def _project_key_option():
    return typer.Option(
        None, "--project-key", help="Clave del proyecto para crear issues (ej. PROJ)."
    )


@app.command()
def setup(
    test_plan_key: str = typer.Option(
        ..., "--test-plan", help="Llave del Test Plan en Jira (ej. PROJ-100)."
    ),
    execution_summary: str = typer.Option(
        ..., "--execution-summary", "-e", help="Titulo para el nuevo Test Execution."
    ),
    application: str = typer.Option(
        ..., "--application", "-a", help="Calificador de aplicacion (ej. APP_WEB)."
    ),
    test_mode: str = typer.Option(
        "clone",
        "--test-mode",
        "-m",
        help="Modo de tests: clone (clonar) o add (agregar sin clonar).",
        metavar="clone|add",
    ),
    config: str | None = _config_option(),
    env: str | None = _env_option(),
    jira_username: str | None = _jira_username_option(),
    jira_password: str | None = _jira_password_option(),
    project_key: str | None = _project_key_option(),
) -> None:
    if not test_plan_key or not execution_summary:
        console.print(
            "[bold red]Error:[/bold red] --test-plan y --execution-summary son obligatorios."
        )
    ...
    table.add_row(
        "Test Executions creadas", ", ".join(str(ex.key) for ex in result.test_executions)
    )
...
@app.command()
def sync(
    execution_key: str = typer.Option(
        ..., "--execution", "-e", help="Llave del Test Execution en Jira (ej. PROJ-200)."
    ),
    folder: str = typer.Option(
        ..., "--folder", "-f", help="Ruta a la carpeta local con los archivos de evidencia."
    ),
    status: str = typer.Option(
        ..., "--status", "-s", help="Estado del test: PASS, FAIL, TODO, EXECUTING, ABORTED."
    ),
    recursive: bool = typer.Option(
        True, "--recursive/--no-recursive", "-r/-R", help="Escanear subcarpetas recursivamente."
    ),
    extensions: str | None = typer.Option(
        None, "--extensions", help="Extensiones permitidas separadas por coma (ej. .jpg,.png)."
    ),
    upload_mode: str = typer.Option(
        "append", "--mode", "-m", help="Modo de subida: append o replace."
    ),
    status_map: str | None = typer.Option(
        None, "--status-map", help="JSON mapeando llaves de test a estados especificos."
    ),
    config: str | None = _config_option(),
    env: str | None = _env_option(),
    jira_username: str | None = _jira_username_option(),
    ...
    console.print(
        f"\n[bold cyan]Sincronizando execution [white]{execution_key}[/white] con carpeta [white]{folder}[/white][/bold cyan]\n"
    )
    try:
        result = pjx.sync(
...
def docs() -> None:
    ...
    if root is None:
        console.print(
            "[bold red]Error:[/bold red] No se encontro documentacion empaquetada ni un proyecto MkDocs local."
        )
        raise typer.Exit(code=1)
    ...
    try:
        proc = subprocess.Popen(
            ["mkdocs", "serve"], cwd=base_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        ...
        while True:
            if proc.poll() is not None:
                console.print(
                    "[bold red]El servidor de MkDocs se detuvo inesperadamente.[/bold red]"
                )
                break
            time.sleep(1)
...
@app.command()
def tui(config: str | None = _config_option()) -> None:
    from ..tui.app import PyJX2App

    PyJX2App(config_file=config).run()
