Feature: Sync Flow
  ...
  Background:
    ...
    And a test execution "PROJ-30" with tests "PROJ-10", "PROJ-11", "PROJ-12"

  Scenario: Files matching test keys are matched to their tests
    Given an evidence folder with files "PROJ-10.png" and "PROJ-11.pdf"
  ...
  Scenario: Recursive scan picks up files in subdirectories
    Given an evidence folder with a nested file "subdir/PROJ-12.png"
  ...
  Scenario: Unmatched tests are reported
    Given an evidence folder with files "PROJ-10.png" only
    When I run the sync command for execution "PROJ-30" with status "PASS"
    Then the unmatched tests include "PROJ-11" and "PROJ-12"

  Scenario: Unmatched files are reported
    Given an evidence folder with files "PROJ-10.png" and "unrelated.txt"
    When I run the sync command for execution "PROJ-30" with status "PASS"
  ...
  Scenario: File matching is case-insensitive for test keys
    Given an evidence folder with files "proj-10.png"
    When I run the sync command for execution "PROJ-30" with status "PASS"
